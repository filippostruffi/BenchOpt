...
- Sentiment Analysis
  - lstm_sentiment
    - Arch: Embedding → LSTM → dropout(0.5) → linear
    - Params: ~1–5M
    - Training: Very fast; sensitive to max_len and embedding init; Adam/AdamW common; gradient clipping recommended
...
- Text Summarization
  - bart_small
    - Arch: Small custom Transformer (encoder–decoder) + sinusoidal pos encodings
    - Params: ~30M (small)
    - Training: Teacher-forcing; ROUGE sensitive to tokenization; AdamW baseline; label smoothing enabled by default
  - tiny_transformer_seq2seq
    ...
    - Training: Lightweight, optimizer-sensitive; good for quick sweeps; label smoothing enabled by default

- Question Answering
  - transformer_qa
    - Arch: Separate Transformer encoders for context and question; mean-pooled q fused into ctx; span heads
    - Params: ~10–20M
    - Training: Start/end CE; LR and WD influence span sharpness; gradient clipping recommended
  - bilstm_attention_qa
    - Arch: BiLSTM encoders; dot attention over question per context token; fusion + span heads
    - Params: ~5–10M
    - Training: Fast, robust; attention sharpness reflects optimizer step quality; gradient clipping recommended
