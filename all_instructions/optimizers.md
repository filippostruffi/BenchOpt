...
### Weight decay heuristics (applied automatically)
The training pipeline sets a default `weight_decay` per optimizer family at runtime. Adjust in code if needed:

- AdamW/Adafactor: 0.01
- SGD family (incl. LARS): 5e-4
- Adam/RAdam/AdaBelief/Yogi/Lion/LAMB: 1e-4
- RMSprop/Adagrad/Adadelta/SAM/GSAM: 1e-4

### Schedulers, early stopping, gradient clipping
- Scheduler: `--scheduler {auto,cosine,plateau,none}` (auto picks cosine for image/segmentation; plateau on val loss for NLP/seq tasks).
- Early stopping: `--early-stopping-patience N` on the task’s primary metric.
- Gradient clipping: `--max-grad-norm X` (global-norm). Defaults to 1.0 for NLP/seq tasks, off for image.

### Outputs per optimizer
For each optimizer run you get:
- `<optimizer>/best_model.pt` (best by primary metric)
- `<optimizer>/<optimizer>_metrics.json` (per-epoch validation metrics)
- Plots and aggregated CSVs at the run root
