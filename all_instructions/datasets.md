...
- Image Classification
  - MNIST
    ...
    - Preprocess: RandomRotation(10) → ToTensor → Normalize((0.1307,), (0.3081,))
  ...
  - CIFAR10
    ...
    - Preprocess (train): RandomCrop(32,padding=4) → RandomHorizontalFlip → ToTensor → Normalize(mean=[0.4914,0.4822,0.4465], std=[0.2470,0.2435,0.2616])
    - Preprocess (val): ToTensor → Normalize(mean=[0.4914,0.4822,0.4465], std=[0.2470,0.2435,0.2616])
...
- Semantic Segmentation
  ...
  - ADE20K
    - Type: Scene parsing with 150 semantic classes
    - Splits: train/validation (we subsample via `--max-samples` if desired)
    - Format: Hugging Face ADE20K (loaded via HF id `scene_parse_150`)
    - Preprocess: Resize to 256×256; ToTensor; Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225]); masks resized with NEAREST; ignore_index=255
    - Why: Rich label space; stresses dense predictions and optimizer behavior
    - Pitfalls: Needs HF `datasets` and cache; small batches can make BN noisy—use drop_last=True (already set)
...
- Text Summarization
  - CNN/DailyMail (3.0.0)
    ...
    - Preprocess: Sequence padding; teacher forcing with `(src, tgt_in, tgt_out)`
...
- Question Answering (extractive)
  - SQuAD v1
    ...
    - Format: HF `squad`
    - Preprocess: Tensorized `(context_ids, question_ids, starts, ends)` for span training
  ...
  - TweetQA
    ...
    - Preprocess: Tensorized `(context_ids, question_ids, starts, ends)`; noisy language increases variance
