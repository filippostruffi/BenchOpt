...
- Image Classification
  ...
  - CSVs: macro_precision, macro_recall, macro_f1 are reported (not micro). Micro values equal accuracy in single-label classification and are omitted to reduce redundancy.
...
- Summary CSV (`*_summary.csv`):
  - Columns include: `task, model, dataset, optimizer, learning_rate, batch_size, primary_metric_name, best_primary_metric_value, best_primary_metric_epoch, runtime_to_best_primary_metric_s, best_val_loss_value, best_val_loss_epoch, runtime_to_best_val_loss_s, runtime_to_convergence_s, epoch_when_convergence_happened, primary_metric_at_convergence, val_loss_at_convergence`.
- Extensive CSV (`*_extensive.csv`):
  - Per-epoch validation rows with: `task, model, dataset, optimizer, epoch, primary_metric_name, primary_metric_value, val_loss, epoch_runtime_s, eval_runtime_s, cumulative_runtime_s`, plus task-specific numeric metrics (macro_* kept; weighted_* and per-class metrics excluded to keep size manageable).
- Per-optimizer JSON: `<optimizer>/<optimizer>_metrics.json` contains per-epoch validation metrics and timing aggregates for that optimizer.
- Per-run text report: `details.txt` summarizes preprocessing, training policy (scheduler, early stopping, clipping), and per-optimizer settings and outcomes.
