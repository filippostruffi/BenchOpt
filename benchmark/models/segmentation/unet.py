...
def build_model(task: Any, dataset: Any) -> nn.Module:
    # Infer input channels and number of classes robustly across a few samples,
    # ignoring the 255 label (commonly used as ignore index).
    x0, y0 = dataset[0]
    in_ch = x0.shape[0]
    max_label = -1
    # Probe up to first 32 samples (or dataset length if smaller)
    probe_count = min(32, len(dataset))
